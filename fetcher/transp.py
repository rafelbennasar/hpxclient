...
class Proxy(threading.Thread):
    ...
    def _get_waitable_lists(self):
        ...
        if self.server and not self.server.closed and self.server.has_buffer():
            logger.debug('connection to server exists and pending server buffer '
                         'found, watching server for write ready')
...
async def configure_server():
    host, port = "127.0.0.1", hpxclient_settings.PROXY_FETCHER_LOCAL_PORT
